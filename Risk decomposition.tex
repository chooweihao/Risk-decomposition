...
\cite{wang1996pct} defines the layer $[k,k+\de k]$ of a random loss $x\ge 0$ as the excess of $x$ over $k$ with the excess capped at $\de k$. Thus the layer is written as
$
L_{k+\de k}(x) - L_k(x)
$
where $L_k(x) \equiv \min(x,k)$ is $x$ capped at $k$. Of interest in this paper are layers over various $k$ for a random loss, therefore drop $x$ from the notation $L_k(x)$ and write the layer $[k,k+\de k]$ of $x$ as $L_{k+\de k}-L_k$, and note layers and capped losses are functions of $x$ and hence random variables.
  
Noting $x$ is the sum of its layers covering $[0,\infty]$ as illustrated in \fref{flayer}, and letting $\de k\rightarrow 0$ yields
\begin{equation}\label{layers}
x= \int_0^\infty \de L_k = \int_0^\infty L_k' \de k = \int_0^\infty (x>k) \de k \;,
\end{equation}
where  $\de L_k$ is the infinitesimally small layer $[k,k+\de k]$ of $x$, $L_k'$ is the derivative of $L_k$ with respect to $k$, and $(x>k)$ is an indicator function equal to 1 if $x>k$ and 0 otherwise. Hence the layer $\de L_k=(x>k) \de k$, an increment $\de k$ if $x$ exceeds $k$. A  proof of \eref{layers} follows by noting the final integral is $\int_0^x \de k = x$.

Equation \eref{layers} decomposes $x$ into infinitely many layers. Each layer is a random variable and is comonotonic with all other layers since $\de L_k$ is non-decreasing in $x$. Integrating these layers over an arbitrary interval yields a larger layer:
$$
\int_a^b \de L_k = L_b-L_a = \min(x,b)-\min(x,a) \;,
$$
in this case the $[a,b]$ layer of $x$, or the excess of $x$ over $a$ where the excess is capped at $b-a$.

Assuming $x$ has distribution function $F$, the mean of layer $\de L_k $ is $\E\{(x>k)\}\de k = \{1-F(k)\} \de k$ where $\E$ is the expectation. \cite{wang1996pct} calls $1-F$ the layer premium density of $x$ as it indicates the mean value or insurance premium of each layer and integrates to the overall mean: $\int_0^\infty \{1-F(k) \}\de k = \E(x)$. \cite{wang1996pct} also distorts the layer premium density, delivering risk-adjusted premiums. Insurance premiums across loss layers are further discussed in \cite{salzmann1963rating}, \cite{evans2001exposure}.
...
This section modifies the layer decomposition of $x$ in \eref{layers} based on the motivating example in \sref{smotivating}. The modified decomposition is central to remaining developments in this paper. Consider a continuous random loss $x\geq 0$ with distribution function $F$ and inverse distribution function $F^-$. For any constant $0\le\alpha\le 1$, the $\alpha$-Value-at-Risk (VaR$_\alpha$) of $x$ is  $V_\alpha\equiv F^-(\alpha)$ and $\alpha$ is the VaR threshold or exceedance probability. Then
...
\end{equation}
where $L_\alpha$ is the capped loss with the cap expressed in terms of VaRs instead of original values as in \eref{layers}. This definition of $L_\alpha$ is applied to remaining developments in this paper. Then $\de L_\alpha$ is the VaR$_\alpha$ layer $[V_\alpha,V_{\alpha+\de\alpha}]$ of $x$, and corresponds to the layer $[\ell_{\alpha n},\ell_{\alpha n+1}]$ of $x$ in \sref{smotivating} assuming $n\rightarrow \infty$. The width of the VaR$_\alpha$ layer is $V_{\alpha+\de\alpha}-V_\alpha \approx V_\alpha' \de \alpha$ and $V_\alpha'$ is called the VaR--spacing,  the continuous analogue to the discrete spacing $\ell_{i+1}-\ell_i$.


Endpoints of VaR layers self-adjust to the shape and scale of the loss distribution. The probability of a positive VaR$_\alpha$ layer is fixed at $\alpha$, hence the mean and risk of VaR layers are comparable across loss distributions. In contrast layers with endpoints expressed in original values as in \sref{sintroduction} are at different probability thresholds depending on the loss distribution.



The $[V_\alpha,V_\beta]$ layer of $x$ integrates VaR layers:
$$
\int_\alpha^\beta \de L_\pi = L_\beta-L_\alpha = \min(V_\beta,x)-\min(V_\alpha,x) \;,
$$
and is again the excess of $x$ over $V_\alpha$ with the excess capped at $V_\beta-V_\alpha$. The $[V_\alpha,V_\beta]$ layer is the insurance or excess-of-loss reinsurance cover on loss $x$ with deductible $V_\alpha$ and limit $V_\beta$. When capital $V_\gamma$ is held against loss $x$, the $[0,V_\gamma]$ layer or $L_\gamma$ is the capital used and the $[V_\gamma,V_1]$ layer or $L_1-L_\gamma$ is the capital shortfall.



\section{The mean density}\label{smean}


The mean density of $x$ is defined analogous to \eref{mean} and indicates the mean value of various VaR layers of $x$:
...
As $\alpha$ increases, $1-\alpha$ decreases reflecting reduced likelihood of $x$ reaching larger VaRs. On the other hand for a right skewed loss distribution where the probability density is decreasing, $V_\alpha'$ increases. The behaviour of the mean density hence depends on relative movements in $1-\alpha$ and $V_\alpha'$. For example, the exponential distribution has a constant mean density whereas the Pareto distribution has an increasing mean density due to more rapidly increasing VaR spacings. These examples are discussed below.

Using \eref{varlayer},
$$
\int_0^1 m_\alpha \de \alpha = \E\left( \int_0^1 \de L_\alpha \right) = \E(x) \;,
$$
hence the total area under the mean density is the overall mean loss. Integrating the mean density over $[\alpha,\beta]$ yields the mean of the $[V_\alpha,V_\beta]$ layer:
...
$$
The mean density increases to infinity as $\alpha\rightarrow 1$:  increasing VaR spacings more than offset decreasing probabilities.
...
$$
Hence the mean density is increasing, or skewness is greater than the exponential, if and only if $(1-\alpha)V_\alpha''/V_\alpha'>1$.
