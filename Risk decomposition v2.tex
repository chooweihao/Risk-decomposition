...
$$
\min\{\max(x-k,0),\de k\} = L_{k+\de k}(x)-L_k(x) \cq L_k(x) \equiv \min(x,k)\;,
$$
where $L_k(x)$ is $x$ capped at $k$. Of interest in this paper are layers over various $k$ for a single random variable $x$, thus drop $x$ from the notation and write the layer $[k,k+\de k]$ of $x$ as $L_{k+\de k}-L_k$. However keep in mind that layers are functions of $x$ and are hence random variables.
